var Editor = new function() {
    ...
    var _this = this,
        ...
        dragresize;
    _this.labels = labels;
    var Label = function(id, type, name, link, data, svg, width, height, left, top, startI, endI) {
        var self = this;
        this.id = id;
        this.name = name;
        this.link = link;
        ...
        if (type != "user") {
            if (_this.type == 'editor' || _this.type == 'create') {
                if (svg == 1) {
                    var labelVideo = $('<div class="video-label {class}" id="{id}"><div class="animate-tag tag1"></div></div>'.replace(/{class}/g, this.is_svg).replace(/{id}/g, "video-label" + this.id));
                } else {
                ...
                if (svg == 1) {
                    var labelVideo = $('<div data-plyr="publish_label" class="video-label publish_link {class}"><a onclick="{ga}" data-save="false" href="{link}"><div class="animate-tag tag1"></div></a></div>'.replace(/{class}/g, class_label).replace(/{id}/g, "video-label" + this.id));
                } else {
            ...
            if (width) {
                ...
                if (svg == 1)
                    labelVideo.find('.animate-tag').css('width', width * coofW + 'px').css('height', height * coofH + 'px');
            } else {
        ...
        instances[0].on('enterfullscreen', function(event) {
            ...
            setTimeout(function() {
                if ($(window).height() / _this.oldH < $(window).width() / _this.oldW) {
                    coof = videoWrap.height() / _this.oldH;
                } else {
                    coof = videoWrap.width() / _this.oldW;
                }
                $('.video-label').each(function(i, elem) {
                    ...
                    /*if($(elem).hasClass("is_svg")){
                      (function($) {
                        $(document).ready(function() {
                          $(elem).find(".animate-tag").destroy();
                          $(elem).find(".animate-tag").width($(elem).width() * coof).height($(elem).height() * coof);
                          $(elem).find(".animate-tag").sprite({fps: 9, no_of_frames: 16})
                                 .active();
                        });
                      })(jQuery);         
                    }*/
            ...
            if (file) {
                ...
                $.ajax({
                    ...
                    success: function(msg) {
                        ...
                        labels[id] = new Label(id, "publisher", name, link, data);
                    }
                });
                $('#labelName2').val(''),
                    $('#labelLink2').val('');
    ...
    _this.init = function() {
        ...
        instances[0].on('canplaythrough', function(event) {
            if (_this.type != "create" && _this.jsonLoad == false) {
                $.get('json_labels.json', {}, function(data) {
                    if (data) {
                       // STOP// data = JSON.parse(data);
                        var i = 0;
                        for (var obj in data) {
                            var id = data[obj].id;
                            if (!data[obj].svg)
