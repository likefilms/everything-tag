var Editor = new function() {
    ...
    var initInterface = function() {
        ...
        var videoPlay = function(e) {
            ...
            else _this.$video[0].play();
            ga('send', 'event', 'videos', 'play', 'Play video: ' + _this.slug);
        }
        // Update current time
	...
	var initFirst=false;
    _this.init = function() {
		console.log('[Editor]: init','initFirst ',initFirst);
		if(!initFirst){
        _this.$video = $(instances[0].getContainer());
        instances[0].on('canplaythrough', function(event) {
			console.log('[Editor]: init','_this.type ',_this.type);
		...
		initFirst=true; //First run finished
		}
    };
};