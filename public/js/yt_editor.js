var Editor = new function() {
  ...
  var Label = function(id, name, link, data, svg, width, height, left, top, startI, endI) {
    ...
    if(width) {
      ...
      if(instances[0].getType() == 'youtube') {
        var mediaWidth = $(instances[0].getMedia()).find("iframe").attr("width");
        var mediaHeight = $(instances[0].getMedia()).find("iframe").attr("height");
      } else {
        var mediaWidth = instances[0].getMedia().videoWidth;
        var mediaHeight = instances[0].getMedia().videoHeight;
      }

      var coofW = _this.$video.width() / mediaWidth,
          coofH = _this.$video.height() / mediaHeight;
